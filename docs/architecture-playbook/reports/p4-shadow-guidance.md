...
P4 门禁通过，并且只开放 P5。P4 将已审秘籍转换为可消费、可验证、可解释的只读影子审查；它不接入主生成流水线，也不授予候选淘汰、设计层控制或返工执行权限。
...
P5 已开放，但必须先有新的设计批准，明确哪些 `satisfied/violated` 结果可以参与候选选择、如何把现有 repair ID 编译为有限设计层 patch、如何保存 checkpoint 与失效关系、失败时如何回到最近 accepted chain，并证明 `playbook=off` 时旧输出不变。
