...
- capability-deny/统一区间计划的已验证代码范围为 `634c03e..cfde94bcc4ef79742912787736d8556f813e8aa4`；`cfde94bcc4ef79742912787736d8556f813e8aa4` 是该计划完成最终审查修复并通过下述验证的固定代码提交，紧随其后的 `c02593c9e86220984ecce0670808cdae9a6c1a77` 仅更新本公开证据报告。最终 URI-component 修复的 TDD RED 在真实子进程环境运行 67 个测试，61 个通过、6 个失败；四个新增受保护快照分别把预期 `1/1/2/2` 计为 `0/0/1/1`。最小分类器修复后同一命令 67/67 通过；补齐 active-UNC raw/encoded 控制后为 69/69。审查修复新增 query→fragment、file→UNC、UNC→file 和 active-file raw/encoded 控制时 RED 为 71/77，统一 file/UNC 高优先级 token 结束模型后为 77/77。
...
- 后续别名能力窄修复的最终固定代码提交为 `0b3330f8418e24a04c4ddef4c0045f585cd6ed3c`。首轮 TDD RED 中，`const fn = () => {}; fn[key](...)` 与 `const R = Reflect; R.get(...)` 均实际执行到 construction，但审计事实为空；独立审查又执行证明 `new`、`.call`、`.apply`、`.bind`、sequence、先取值后调用和 tagged template 七种同源变体仍可绕过，同时发现 `Reflect['apply']` 误报。最终实现只记录直接 `const` 函数或类表达式的词法来源，未知 computed 属性在取值源头即被拒绝；未遮蔽全局 `Reflect` 只在作为可转移值或动态属性根时拒绝。实现不做跨赋值 taint 传播，也不对白名单目录放行。精确回归连同局部同名绑定、普通动态分发表、静态 computed 调用、`Reflect.apply` 与 `Reflect['apply']` 对照为 23/23；七文件聚焦门禁为 325/325，完整 `npm test` 为 847/847；受管检查仍为 `current`、5 个产物、漂移数 0。
