...
- 后续别名能力窄修复的固定代码提交为 `09f84afcea3b6472b187d72128b956309b16765b`。TDD RED 中，`const fn = () => {}; fn[key](...)` 与 `const R = Reflect; R.get(...)` 均实际执行到 construction，但审计事实为空；修复后精确回归连同局部同名绑定、普通动态分发表、静态 computed 调用和 `Reflect.apply` 对照为 15/15。实现只记录直接 `const` 函数表达式的词法来源，并拒绝未遮蔽全局 `Reflect` 作为可转移值，不做跨赋值 taint 传播，也不对白名单目录放行。七文件聚焦门禁为 317/317，完整 `npm test` 为 839/839；受管检查仍为 `current`、5 个产物、漂移数 0。
