...
这些 prompt 适合用来生成课程报告或 GitHub 展示页里的真实运行截图。优先使用 `--prompt-id`，因为推荐 prompt 自带固定 seed，结果更容易复现。
...
本次提交已补充四组 Minecraft 真实截图：

| 组别 | Prompt | 截图 |
| --- | --- | --- |
| A | 现代两层家庭别墅 | `A1`、`A2` 为外景，`A3` 为内饰 |
| B | 日式一层庭院住宅 | `B1`、`B2` 为外景，`B3` 为内饰 |
| C | 欧式三层大别墅 | `C1`、`C2` 为外景，`C3` 为内饰 |
| D | 海滨架空度假住宅 | `D1`、`D2` 为外景，`D3` 为内饰 |

如果后续继续补图，每次运行后，终端会打印输出目录。建议优先补三类图：
