...
test('pure public leak audit uses bounded percent-normalized file, UNC, and HTTPS ranges', async () => {
  ...
  const leakCases = [
    ...
    ['embedded UNC in HTTPS path', String.raw`https://example.test/path/\\server\share\a.txt`, 1],
    ['embedded mixed UNC in HTTPS path', String.raw`https://example.test/path=/\server\share\a.txt`, 1],
    ['embedded UNC in HTTPS query', String.raw`https://example.test/?next=\\server\share\a.txt`, 1],
    ['embedded mixed UNC in HTTPS fragment', String.raw`https://example.test/#next=/\server\share\a.txt`, 1],
    ['two file URLs in one HTTPS query', 'https://example.test/?a=file:/one&a=file:/two', 2],
    ['two UNC references in one HTTPS query', String.raw`https://example.test/?a=\\one\share&a=\\two\share`, 2],
    ['two comma-separated UNC references', String.raw`\\one\share,\\two\share`, 2],
...
test('pure public leak audit exempts only the HTTPS scheme delimiter', async () => {
  ...
  for (const [name, reference, expectedCount] of [
    ...
    ['scheme-token suffix does not create an HTTPS exception',
      'x-https://example.test/home/alice/private.txt', 1],
    ['normalized scheme-token suffix does not create an HTTPS exception',
      'x-https:%2F%2Fexample.test%2Fhome%2Falice%2Fprivate.txt', 1],
