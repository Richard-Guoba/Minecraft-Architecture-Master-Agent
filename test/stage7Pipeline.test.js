...
test('Stage 7 defaults off and explicit off preserve exact fixed-seed operations', async () => {
  ...
  try {
    ...
    const explicitOff = await runFixture(
      path.join(root, 'explicit-off'),
      { coarseVoxelMode: 'off' }
    );
    assert.deepEqual(
      explicitOff.blueprint.operations,
      baseline.blueprint.operations
    );
...
test('Stage 7 baseline shadow writes review artifacts without changing primary operations', async () => {
  ...
  try {
    ...
    for (const key of [
      ...
      assert.ok(shadow.artifacts[key]);
    ...
    const stage7Report = await fs.readFile(
      shadow.artifacts.stage7Report,
      'utf8'
    );
    const savedBlueprint = JSON.parse(
      await fs.readFile(shadow.artifacts.blueprint, 'utf8')
    );
    assert.match(report, /## Stage 7 Milestone 1 Shadow/);
    assert.match(stage7Report, /Primary geometry changed: no/);
    assert.equal(
      savedBlueprint.stage7.condition_hash,
      shadow.stage7.condition.condition_hash
    );
  } finally {
    await fs.rm(root, { recursive: true, force: true });
  }
...
test('invalid Stage 7 artifacts reject while normal build succeeds unchanged', async () => {
  const root = await fs.mkdtemp(
    path.join(os.tmpdir(), 'stage7-rejected-pipeline-')
  );
  try {
    ...
    assert.equal(
      rejected.blueprint.stage7.fallback,
      'primary-build-unchanged'
    );
    assert.deepEqual(
      rejected.blueprint.operations,
      baseline.blueprint.operations
    );
    assert.ok(rejected.artifacts.stage7FailureCase);
    assert.ok(rejected.artifacts.stage7RawPlan);
    assert.equal(
      await fs.readFile(rejected.artifacts.stage7RawPlan, 'utf8'),
      await fs.readFile(artifactPath, 'utf8')
    );
    const failure = JSON.parse(
      await fs.readFile(rejected.artifacts.stage7FailureCase, 'utf8')
    );
    assert.equal(
      failure.baseline_artifact_paths.blueprint,
      'blueprint.json'
    );
    assert.equal(
      failure.baseline_artifact_paths.run_report,
      'run_report.md'
    );
    assert.equal(
      failure.diagnostic_artifact_paths.raw_plan,
...
test('artifact provider rejects multi-candidate execution', async () => {
  await assert.rejects(
    runPipeline({
      prompt: PROMPT,
      mode: 'mock',
      outRoot: path.join(os.tmpdir(), 's7'),
      seed: 7101,
      candidates: 2,
      coarseVoxelMode: 'shadow',
      coarseVoxelProvider: 'artifact',
  ...
  await assert.rejects(
    runCandidatePipeline({
      prompt: PROMPT,
      mode: 'mock',
      outRoot: path.join(os.tmpdir(), 's7c'),
      seed: 7101,
      candidates: 2,
      coarseVoxelMode: 'shadow',
      coarseVoxelProvider: 'artifact',
