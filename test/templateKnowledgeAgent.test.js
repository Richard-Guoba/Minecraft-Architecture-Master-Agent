...
test('TemplateKnowledgeAgent falls back when neural retrieval is enabled without an embedding index', async () => {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), 'mc-template-agent-neural-missing-index-'));
  try {
    await writeAgentAnalysisFixture(root, { includeEmbeddingIndex: false });
    const agent = new TemplateKnowledgeAgent({
      cwd: root,
      analysisFile: path.join(root, 'mc_templates', 'analysis', 'template_index.json'),
      neuralRetrieval: true
    ...
    assert.equal(result.retrieval_explanation.source, 'template-explainable-retriever-v1');
    assert.equal(result.retrieval_explanation.mode, 'rule-only-fallback');
    assert.equal(result.retrieval_explanation.fallback_used, true);
    assert.match((result.retrieval_explanation.warnings || []).join(' '), /embedding index missing/i);
  } finally {
    await fs.rm(root, { recursive: true, force: true });
  }
});

test('TemplateKnowledgeAgent surfaces invalid neural label artifacts during opted-in neural retrieval', async () => {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), 'mc-template-agent-neural-invalid-labels-'));
  try {
    await writeAgentAnalysisFixture(root, { neuralLabels: 'invalid' });
    const agent = new TemplateKnowledgeAgent({
      cwd: root,
      analysisFile: path.join(root, 'mc_templates', 'analysis', 'template_index.json'),
      neuralRetrieval: true
    ...
    assert.equal(result.retrieval_explanation.mode, 'rule-only-fallback');
    assert.equal(result.retrieval_explanation.fallback_used, true);
    assert.match((result.retrieval_explanation.warnings || []).join(' '), /neural labels/i);
  } finally {
    await fs.rm(root, { recursive: true, force: true });
  }
});

test('TemplateKnowledgeAgent keeps rule-only retrieval when neural retrieval is explicitly disabled', async () => {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), 'mc-template-agent-neural-disabled-'));
  try {
    await writeAgentAnalysisFixture(root);
    const agent = new TemplateKnowledgeAgent({
      cwd: root,
    ...
    assert.equal(result.retrieval_explanation.source, 'template-explainable-retriever-v1');
    assert.equal(result.retrieval_explanation.mode, 'rule-only');
    assert.notEqual(result.retrieval_explanation.source, 'stage5-neural-template-retriever-v1');
  } finally {
    await fs.rm(root, { recursive: true, force: true });
  }
});

async function writeAgentAnalysisFixture(root, { includeEmbeddingIndex = true, neuralLabels = 'omit' } = {}) {
  ...
  const labelRecords = neuralLabels === 'valid' ? [validNeuralLabelFixture()] : [];
  ...
  if (neuralLabels === 'valid') {
    await fs.writeFile(path.join(analysisDir, 'neural_labels.jsonl'), `${JSON.stringify(validNeuralLabelFixture())}\n`, 'utf8');
  } else if (neuralLabels === 'invalid') {
    await fs.writeFile(path.join(analysisDir, 'neural_labels.jsonl'), '{"case_id": "house-modern-lake-villa"\n', 'utf8');
  }
}
