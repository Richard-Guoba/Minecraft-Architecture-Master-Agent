...
function validateCorpus(cards, admissionDocument, coverageDocument) {
  ...
  for (const [index, card] of cards.entries()) {
    ...
    if (card.playbook_version !== PLAYBOOK_VERSION || card.primary_school !== SCHOOL_ID) invalid();
    ...
    if (admission.decision !== card.admission_status) invalid();
...
function validateCoverage(layers, cards) {
  ...
  for (const [index, layer] of layers.entries()) {
    ...
    if (layer.runtime_authority !== 'none' || !Array.isArray(layer.rule_ids)) invalid();
