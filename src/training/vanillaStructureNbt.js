import { BOUNDED_NBT_LIMITS, BOUNDED_NBT_VERSION } from './boundedNbt.js';
...
export function isAirIdentifier(value) {
  return AIR.has(value);
}
...
export function validateVanillaStructureNbt(decoded, {
  ...
  if (
    root.palette.length === 0
    || root.palette.length > limits.maxPaletteEntries
  ) {
    fail('STRUCTURE_PALETTE_LIMIT', id, {
      palette_count: root.palette.length
    });
  }
  ...
  const palette = Object.freeze(
    root.palette.map((entry) => paletteEntry(entry, id))
  );
  const seen = new Set();
  ...
  const blocks = root.blocks.map((entry) => {
    if (
      !plain(entry)
      || !Number.isSafeInteger(entry.state)
      || !Array.isArray(entry.pos)
      || entry.pos.length !== 3
      || !entry.pos.every(Number.isSafeInteger)
    ) {
      fail('STRUCTURE_BLOCK_INVALID', id);
    }
    if (entry.state < 0 || entry.state >= palette.length) {
      fail('STRUCTURE_PALETTE_INDEX_INVALID', id, {
        palette_index: entry.state
      });
    }
    ...
    const key = `${x},${y},${z}`;
    if (seen.has(key)) {
      fail('STRUCTURE_COORDINATE_DUPLICATE', id, { x, y, z });
    }
    seen.add(key);
    ...
    if (blockEntityPresent && !plain(entry.nbt)) {
      fail('STRUCTURE_BLOCK_ENTITY_INVALID', id);
    }
    ...
    return Object.freeze({
      ...
      z,
      palette_index: entry.state,
      block_entity_present: blockEntityPresent
    });
  });
  if (blockEntityCount > limits.maxBlockEntities) {
    fail('STRUCTURE_BLOCK_ENTITY_LIMIT', id, {
      block_entity_count: blockEntityCount
    });
  }
  const nonAir = blocks.filter(
    (block) => !isAirIdentifier(palette[block.palette_index].name)
  );
  ...
  return Object.freeze({
    ...
    block_entity_count: blockEntityCount,
    non_air_bounds: boundsOf(nonAir)
  });
}

function dimensions(value, sourceId) {
  if (
    value.length !== 3
    || !value.every((item) => Number.isSafeInteger(item) && item > 0)
  ) {
    fail('STRUCTURE_SIZE_INVALID', sourceId);
  }
  return { x: value[0], y: value[1], z: value[2] };
}

function paletteEntry(entry, sourceId) {
  if (
    !plain(entry)
    || typeof entry.Name !== 'string'
    || !RESOURCE_ID.test(entry.Name)
    || entry.Name.split(':')[1].split('/').includes('..')
  ) {
    fail('STRUCTURE_BLOCK_ID_INVALID', sourceId);
  }
  const properties = entry.Properties === undefined ? {} : entry.Properties;
  if (
    !plain(properties)
    || Object.entries(properties).some(
      ([key, value]) => !PROPERTY_NAME.test(key)
        || typeof value !== 'string'
        || !PROPERTY_VALUE.test(value)
    )
  ) {
    fail('STRUCTURE_PROPERTIES_INVALID', sourceId);
  }
...
function fail(code, sourceId, metadata = {}) {
  throw new TrainingDataError(code, `structure:${sourceId}`, {
    stage: 'structure',
    source_id: sourceId,
    ...metadata
  });
