...
export const MATERIAL_MAPPING_VERSION = 'categorical-material-map-v1';
...
const COMPILED_RULES = RULES.map((rule) => ({
  token: rule.token,
  pattern: new RegExp(rule.source, 'u')
}));

export const MATERIAL_MAPPING_SHA256 = createHash('sha256')
  .update(JSON.stringify({ version: MATERIAL_MAPPING_VERSION, rules: RULES }))
  .digest('hex');

export function mapStructureMaterial(identifier) {
  if (isAirIdentifier(identifier)) {
    return Object.freeze({ token: 0, mapped: true });
  }
...
export function prepareStructureVolume({
  sourceId,
  contentSha256,
  volume
}) {
  ...
  if (!/^[a-f0-9]{64}$/u.test(contentSha256 || '')) {
    fail('CONTENT_HASH_INVALID', id);
  }
  if (
    !volume
    || volume.version !== VANILLA_STRUCTURE_ADAPTER_VERSION
    || volume.source_id !== id
  ) {
    fail('PREPARATION_INPUT_INVALID', id);
  }
  const extent = volume.non_air_bounds.extent;
  if ([extent.x, extent.y, extent.z].some((value) => value > GRID)) {
    fail('VOLUME_TOO_LARGE', id, {
      x: extent.x,
      y: extent.y,
      z: extent.z
    });
  ...
  for (const block of volume.blocks) {
    ...
    const mapping = mapStructureMaterial(palette.name);
  ...
  const binding = {
    ...
    mapping_version: MATERIAL_MAPPING_VERSION,
    mapping_sha256: MATERIAL_MAPPING_SHA256,
  ...
  const preparationSha256 = createHash('sha256')
...
function fail(code, sourceId, metadata = {}) {
  throw new TrainingDataError(code, `preparation:${sourceId}`, {
    stage: 'preparation',
    source_id: sourceId,
    ...metadata
  });
