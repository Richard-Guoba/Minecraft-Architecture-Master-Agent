...
export function parseStage7DatasetArgs(argv=[]) {
  const options={root:'mc_templates',knowledgeBase:'',out:'',localArtifacts:'',datasetVersion:'v1',reviewOverlay:'',caseIds:[],requireEligible:0,help:false};
...
export async function main(argv=process.argv.slice(2)) {
  ...
  const result=await writeStage7DatasetArtifacts({
    ...
    caseIds:options.caseIds,requireEligible:options.requireEligible
  });
