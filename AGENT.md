...
- 当前 v3 重点：命令行多智能体原型，不是实时 Mineflayer 游戏机器人。
- 核心演示流程：用户输入需求 -> 需求解析 -> 建筑 skill 路由 -> 语义建筑规划 -> 房屋设计 -> 蓝图生成 -> 自动校验 -> 软质量评审 -> 一次轻量修正 -> Minecraft 数据包导出。
...
- 已实现：Node.js ESM 命令行入口、需求解析 Agent、skill 路由 Agent、语义规划 Agent、设计 Agent、蓝图生成 Agent、校验 Agent、Critic 评审 Agent、Repair 修正 Agent、导出 Agent、Minecraft 1.21 数据包输出、自动安装到本地世界、数据包自动建造、本地 HTML 预览和测试。
- 已实现的主要演示风格：欧式大体量、现代两层、江南/中式小院；其中欧式和现代会初步生成规划侧翼，江南/中式会生成庭院入口轴线和水景规划。
- 部分实现：LLM 可以参与需求解析、skill 路由、语义建筑规划和软质量评审；复杂 footprint、约束求解、体素占用冲突检测、多轮自我修正和真实 MCP 工具服务仍是后续增强方向。
