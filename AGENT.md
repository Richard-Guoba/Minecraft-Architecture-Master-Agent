...
- 核心演示流程：用户输入需求 -> 需求解析 -> 语义建筑规划 -> 房屋设计 -> 蓝图生成 -> 自动校验 -> Minecraft 数据包导出。
...
- 已实现：Node.js ESM 命令行入口、需求解析 Agent、语义规划 Agent、设计 Agent、蓝图生成 Agent、校验 Agent、导出 Agent、Minecraft 1.21 数据包输出、自动安装到本地世界、数据包自动建造、本地 HTML 预览和测试。
- 已实现的主要演示风格：欧式大体量、现代两层、江南/中式小院；其中欧式和现代会初步生成规划侧翼，江南/中式会生成庭院入口轴线和水景规划。
- 部分实现：LLM 可以参与需求解析和语义建筑规划，但复杂 footprint、约束求解、体素占用冲突检测和多轮自我修正仍是后续增强方向。
